...
class FastSpeech2(nn.Module):
    def __init__(self, config):
        ...
        # modify is_emotion_feature
        if hasattr(config, "is_emotion_feature"):
            self.is_emotion_feature = config.is_emotion_feature
            self.emotion_emb = nn.Linear(512, config.emotion_emb_hidden_size)
        else:
            self.is_emotion_feature = False
    ...
    def forward(self, device, batch_dict) -> dict:
        ...
        if self.is_emotion_feature:
            emotion_embedding = self.emotion_emb(batch_dict["emotion_features"].to(device))
        else:
