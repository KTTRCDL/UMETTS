...
class Tacotron2(nn.Module):
    def __init__(self, hparams):
        ...
        # modify add multi_speaker and multi_emotion
        ...
        if self.is_multi_emotion and self.is_emotion_feature:
            self.emotion_embedding = nn.Linear(
                hparams.emotion_feature_dim, hparams.emotion_embedding_dim)
            torch.nn.init.xavier_normal_(self.emotion_embedding.weight)
            hparams.encoder_embedding_dim = hparams.encoder_embedding_dim + hparams.emotion_embedding_dim
        elif self.is_multi_emotion:
    ...
    def parse_batch(self, batch):
        # modify batch to include speaker_ids and emotion_ids
        if self.is_multi_speaker and self.is_multi_emotion and self.is_emotion_feature:
            text_padded, input_lengths, mel_padded, gate_padded, \
                output_lengths, speaker_ids, emotion_ids, emotion_features = batch
            ...
            emotion_features = to_gpu(emotion_features).float()

            return (
                (text_padded, input_lengths, mel_padded, max_len, output_lengths, speaker_ids, emotion_ids, emotion_features),
    ...
    def forward(self, inputs):
        ...
        # modify inputs to include speaker and emotion
        if self.is_multi_speaker and self.is_multi_emotion and self.is_emotion_feature:
            text_inputs, text_lengths, mels, max_len, output_lengths, speaker_ids, emotion_ids, emotion_features = inputs
        elif self.is_multi_speaker and self.is_multi_emotion:
        ...
        if self.is_multi_emotion and self.is_emotion_feature:
            emotion_embeds = emotion_features.unsqueeze(1)
            embedded_emotions = self.emotion_embedding(emotion_embeds)
            ...
            embedded_emotions = self.emotion_embedding(emotion_ids)
            embedded_emotions = embedded_emotions.expand(-1, max_len, -1)
            outputs.append(embedded_emotions)
    ...
    def inference(self, inputs, speaker_id=None, emotion_id=None, emotion_feature=None):
        ...
        if emotion_feature != None:
            emotion_embed = emotion_feature.unsqueeze(1)
            embedded_emotion = self.emotion_embedding(emotion_embed)
            embedded_emotion = embedded_emotion.expand(-1, encoder_outputs.shape[1], -1)
            outputs.append(embedded_emotion)
        elif emotion_id != None:
            emotion_id = emotion_id.unsqueeze(1)
            embedded_emotion = self.emotion_embedding(emotion_id)
            embedded_emotion = embedded_emotion.expand(-1, encoder_outputs.shape[1], -1)
            outputs.append(embedded_emotion)
